...
export const addBasketItemAsync = createAsyncThunk<Basket, {productId: number, quantity?: number}>( //Basket=tipul parametrilor returnati + productId/ quantity=tipul argumentelor
    'basket/addBasketItemAsync', //numele functiei care o sa apara in Redux dev tools
    async ({productId, quantity}) => { //parametrii care trebuie pasati ca argument metodei asincrone => API request
        try {
